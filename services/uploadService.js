...
function createUploadService(config, boardService) {
  return {
    saveUploadedFiles: (request) => saveUploadedFiles(request, config),
    deleteUploadedFile: (fileName) => deleteUploadedFile(fileName, config, boardService),
    safeUploadFileName
  };
}

async function saveUploadedFiles(request, config) {
...
async function deleteUploadedFile(fileName, config, boardService) {
